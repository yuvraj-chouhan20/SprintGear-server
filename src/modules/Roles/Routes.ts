...
export default function(app: Application){
  ...
  router.post("/roles", Validator.validateGetRoles, Auth.isAuthorized, async( req: RequestType, res: Response, next: NextFunction) =>{
    const roleObj: RoleController = new RoleController(req, res, next);
    await roleObj.getRoleListing();
  })

  router.post("/roles/update/:roleId", Validator.validateUpdateRole, Auth.isAuthorized, async (req: RequestType, res: Response, next: NextFunction) => {
    const roleObj: RoleController = new RoleController(req, res, next);
    await roleObj.updateRole();
  })

  router.delete("/roles/delete/:roleId", Auth.isAuthorized, async (req: RequestType, res: Response, next: NextFunction) => {
    const roleObj: RoleController = new RoleController(req, res, next);
